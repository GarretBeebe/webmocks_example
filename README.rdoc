...
* Create config/dotenv/.env by adding your db credentials.
    ...
    DEVELOPMENT_DATABASE=skeleton_rails_development
...
* Create config/dotenv/.env.test by adding the following:
    ...
    TEST_HOST=10.0.2.2
    TEST_DATABASE=skeleton_rails_test
...
* A postman collection for api testing is included in the root of the project.

* Before any development begins, find all references to "skeleton-rails-project" and "skeleton_rails". Replace with new project name following preferred naming conventions for file type and location.
